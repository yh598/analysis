...
Cosmos Gremlin is picky about `select(values)` and `count(local)`.  
Use `select(Column.values)` and `count(Scope.local)` instead.
...
g.V().hasLabel('claim').
  ...
  group().
    ...
    by(select('c').fold()).
  unfold().
  filter(select(Column.values).count(Scope.local).is(P.gt(1))).
...
Cosmos friendly version uses `P.gt()` and avoids ambiguous `gt()`.

```gremlin
g.V().hasLabel('phone').
  filter(in('shares_phone').hasLabel('member').dedup().count().is(P.gt(1))).
  filter(in('shares_phone').hasLabel('provider').dedup().count().is(P.gt(0))).
...
Cosmos friendly version uses `P.gt()`.

```gremlin
g.V().hasLabel('bank').
  filter(in('shares_bank').hasLabel('provider').dedup().count().is(P.gt(1))).
...
Cosmos friendly version uses `Column.values` and `P.gt()`.
...
This one is already Cosmos friendly, just keep it simple.
...
Cosmos friendly viewer uses `choose` to avoid `fold()` for missing properties.
...
g.V().hasLabel('claim').
  project('claimId','memberId','providerId','serviceDate','amount','ui_color','risk_reason','risk_rule','risk_score').
    ...
    by(values('serviceDate')).
    by(values('amount')).
    by(choose(values('ui_color').count().is(P.gt(0)), values('ui_color'), constant('missing'))).
    by(choose(values('risk_reason').count().is(P.gt(0)), values('risk_reason'), constant(''))).
    by(choose(values('risk_rule').count().is(P.gt(0)), values('risk_rule'), constant(''))).
    by(choose(values('risk_score').count().is(P.gt(0)), values('risk_score'), constant(0)))
```
